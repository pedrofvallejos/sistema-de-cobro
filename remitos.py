...
import os
import platform  # <-- NUEVA IMPORTACIÓN PARA ABRIR EL PDF
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.title("Francisco - Sistema v8.9 (Control de Mora)")
        ...
        self.scroll_clientes = ctk.CTkScrollableFrame(self.frame_lista, fg_color="transparent")
        self.scroll_clientes.pack(padx=20, pady=5, fill="both", expand=True)

        # --- PANEL DERECHO ---
        self.frame_detalle = ctk.CTkFrame(self, fg_color="#1a1a1a", border_width=1, border_color="#1f538d")
        self.frame_detalle.grid(row=0, column=1, padx=20, pady=20, sticky="nsew")
        self.lbl_info = ctk.CTkLabel(self.frame_detalle, text="Seleccione un cliente", font=("Segoe UI", 16), justify="left")
        self.lbl_info.pack(pady=30, padx=30, fill="both")
        ...
        self.btn_nuevo = ctk.CTkButton(self.frame_detalle, text="+ NUEVO CLIENTE", fg_color="#2e7d32", height=50, command=self.ventana_agregar)
        self.btn_nuevo.pack(side="bottom", pady=30, fill="x", padx=40)

        self.dni_sel = None
        self.actualizar_lista()

    def calcular_mora(self, fila):
        """Calcula fechas de vencimiento y atraso (Corregido el límite de cuotas)"""
        ...
        try:
            ...
            # Si ya pagó todas las cuotas, no hay mora ni cuotas adeudadas
            if pagas >= totales:
                return f_carga, hoy, 0, 0
            
            # La primera cuota se paga al comprar (pagas=1), la segunda vence después de 1 frecuencia
            if frec == "Semanal": 
            ...
            # Calculamos cuántas cuotas se pasaron de la fecha
            if dias_atraso >= 0:
                cuotas_debe = (dias_atraso // dias_ciclo) + 1
                
                # --- NUEVO FRENO ---
                # Las cuotas que debe no pueden superar las cuotas que le faltan pagar
    ...
    def generar_remito_pdf(self):
        """Genera un archivo PDF formato remito para el cliente seleccionado"""
        if not self.dni_sel:
            return
        ...
        # Guardar el archivo dentro de la carpeta con el nombre del cliente
        ...
        # 2. DATOS DEL CLIENTE Y LA COMPRA
        ...
        for i in range(1, int(c['Cuotas Totales']) + 1):
            if i == 1:
                ...
                if frec == "Semanal": 
                    f_venc = f_carga + timedelta(weeks=i-1)
                elif frec == "Quincenal": 
                    f_venc = f_carga + timedelta(days=(i-1) * 15)
                else: 
                    f_venc = f_carga + relativedelta(months=i-1)
            
            estado = "PAGADA" if i <= int(c['Cuotas Pagas']) else ""
        ...
        # 5. CLÁUSULA DE GARANTÍA Y FIRMAS
        ...
        # --- NUEVO: CÓDIGO PARA ABRIR EL PDF EN PANTALLA ---
        try:
            ...
            messagebox.showwarning("Aviso", "No se pudo abrir el PDF automáticamente, pero se guardó con éxito.")
            
        messagebox.showinfo("Éxito", f"Se ha generado la boleta correctamente en la carpeta Boletas:\n{nombre_archivo}")
