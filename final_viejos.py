...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.title("Francisco - Sistema v14.0")
    ...
    def generar_remito_pdf(self):
        ...
        style_garantia = ParagraphStyle('Garantia', parent=style_normal, fontName='Helvetica', fontSize=8, leading=11)

        # SECCIÓN LOGO INTELIGENTE PARA EL .EXE
        ...
        # 🚨 SECCIÓN MODIFICADA DE GARANTÍAS SEGÚN IMAGEN 🚨
        story.append(Paragraph("<b>INFORMACION DE GARANTIAS:</b>", style_subtitulo))
        story.append(Spacer(1, 0.15*cm))
        
        texto_g1 = "• Los productos que no necesiten instalacion tienen periodo de prueba <b>48hs</b> posterior al retiro, pasado este periodo pasan a la garantia oficial <b><u>SIN EXCEPCION</u></b>."
        story.append(Paragraph(texto_g1, style_garantia))
        story.append(Spacer(1, 0.1*cm))
        
        texto_g2 = "• Los productos que requieran instalacion (Ej: termotanques, split, etc) deben presentar factura del <b><u>instalador matriculado original</u></b>."
        story.append(Paragraph(texto_g2, style_garantia))
        story.append(Spacer(1, 0.1*cm))
        
        texto_g3 = "• El producto <b><u>NO</u></b> debe de ser desinstalado y pasaran a garantia oficial <b><u>SIN EXCEPCION</u></b>."
        story.append(Paragraph(texto_g3, style_garantia))
        story.append(Spacer(1, 0.1*cm))
        
        texto_mora = "• Transcurridos los 30 dias de atraso en las cuotas, se generaran recargos punitorios. En caso de incumplimiento con el pago acordado, el garante debe asumir el cargo de los pagos adeudados o bien devolver el producto."
        story.append(Paragraph(texto_mora, style_garantia))
    ...
    def ventana_agregar(self):
        ...
        ctk.CTkLabel(sf, text="──────────────────────────────", text_color="#555555").pack(pady=10)
        
        es_vieja_v = ctk.BooleanVar(value=False)
        
        def toggle_campos_viejos():
            if es_vieja_v.get():
                e_fecha.configure(state="normal", fg_color="#2b2b2b")
                e_pagas.configure(state="normal", fg_color="#2b2b2b")
            else:
                e_fecha.delete(0, "end")
                e_fecha.insert(0, datetime.now().strftime("%d/%m/%Y"))
                e_fecha.configure(state="disabled", fg_color="#1b1b1b")
                e_pagas.delete(0, "end")
                e_pagas.insert(0, "1")
                e_pagas.configure(state="disabled", fg_color="#1b1b1b")
            recalcular()

        chk_vieja = ctk.CTkCheckBox(sf, text="¿Es una venta vieja / anterior?", variable=es_vieja_v, command=toggle_campos_viejos)
        chk_vieja.pack(pady=5)

        ctk.CTkLabel(sf, text="Fecha de Compra (DD/MM/AAAA) *").pack(pady=(5,0))
        e_fecha = ctk.CTkEntry(sf, width=300, placeholder_text="Ej: 15/11/2025")
        e_fecha.pack(pady=2)
        e_fecha.insert(0, datetime.now().strftime("%d/%m/%Y"))
        e_fecha.configure(state="disabled", fg_color="#1b1b1b")

        ctk.CTkLabel(sf, text="¿Cuántas cuotas ya pagó? *").pack(pady=(5,0))
        e_pagas = ctk.CTkEntry(sf, width=300)
        e_pagas.pack(pady=2)
        e_pagas.insert(0, "1")
        e_pagas.configure(state="disabled", fg_color="#1b1b1b")
        ...
        def formatear_fecha_en_vivo(event):
            ...
            for i, num in enumerate(solo_numeros):
                if i == 2 or i == 4: formateado += "/"
                formateado += num
            e_fecha.delete(0, "end")
            e_fecha.insert(0, formateado)
        ...
        def guardar():
            ...
            if not e_fecha.get().strip() or not e_pagas.get().strip():
                return messagebox.showwarning("Falta", "Completar los campos de la venta vieja.")

            try:
                p = float(limpiar_monto(entradas["Precio"].get()))
                c = int(entradas["Cuotas"].get())
                v_c = p / c
                c_pagas = int(e_pagas.get())
                
                if c_pagas > c:
                    return messagebox.showerror("Error", "Las cuotas pagas no pueden ser mayores a las totales.")
                
                fecha_ingresada = e_fecha.get().strip()
                fecha_objeto = datetime.strptime(fecha_ingresada, "%d/%m/%Y")
                fecha_formato_guardado = fecha_objeto.strftime("%Y-%m-%d")
                
            except ValueError:
                return messagebox.showerror("Error", "Revisá los números ingresados o que la fecha sea válida (DD/MM/AAAA).")
            ...
            nueva_fila = {
                ...
                "Cuotas Totales": c, 
                "Valor Cuota": v_c, 
                "Cuotas Pagas": c_pagas, 
                "Saldo": p - (v_c * c_pagas), 
                "Frecuencia": frec_v.get(), 
                "Fecha Carga": fecha_formato_guardado
            }
