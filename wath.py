...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.title("Francisco - Sistema v12.0 (Stuar's Alerts Edition)")
        ...
        self.tab_listas.add("🧮 Calculadora de Mora") # NUEVA PESTAÑA
        ...
        # Contenido de la nueva pestaña Calculadora
    ...
    def ver_detalle(self, index):
        ...
        # Cargamos los datos en la pestaña de la Calculadora de Mora
    ...
    def registrar_pago_con_punitorio(self):
        ...
        if messagebox.askyesno("Cobro con Mora", f"¿Registrar pago total de {format_moneda_visual(total)}?\n(Cuota: {format_moneda_visual(v_cuota)} + Punitorio: {format_moneda_visual(punitorio)})"):
            # En la base de datos solo restamos la cuota pura, el punitorio es ganancia neta por mora
            self.df_datos.at[self.index_sel, "Saldo"] -= v_cuota
            self.df_datos.at[self.index_sel, "Cuotas Pagas"] += 1
            self.df_datos.to_excel(ARCHIVO_EXCEL, index=False)
            self.actualizar_lista(); self.ver_detalle(self.index_sel)
            
            # Avisamos que el pago impactó
    ...
    def enviar_whatsapp(self):
        ...
        mensaje = (f"Hola *{c['Nombre']}*!\n\n"
                   f"Registramos tu pago de *{format_moneda_visual(c['Valor Cuota'])}* en *Stuar's*.\n"
    ...
    def enviar_whatsapp_con_punitorio(self, punitorio, total):
        ...
        mensaje = (f"Hola *{c['Nombre']}*!\n\n"
                   f"Registramos tu pago por un total de *{format_moneda_visual(total)}* en *Stuar's*.\n"
    ...
    def enviar_aviso_vencimiento(self):
        ...
        mensaje = (f"Hola *{c['Nombre']}*!\n\n"
                   f"Te recordamos de *Stuar's* que se aproxima el vencimiento de tu cuota.\n"
    ...
    def enviar_aviso_mora(self):
        ...
        # NUEVA VERSIÓN: Sin montos para poder cobrar punitorios en persona/chat
        mensaje = (f"Hola *{c['Nombre']}*! ATENCION\n\n"
                   f"Nos comunicamos de *Stuar's* porque registramos un atraso en tu cuenta.\n"
    ...
    def ventana_agregar(self):
        vent = ctk.CTkToplevel(self); vent.geometry("500x850"); vent.title("Nuevo Crédito"); vent.attributes("-topmost", True)
        sf = ctk.CTkScrollableFrame(vent, fg_color="transparent"); sf.pack(fill="both", expand=True, padx=10, pady=10)
        entradas = {}
        for campo in ["DNI", "Nombre", "Domicilio", "Telefono", "Producto", "Precio", "Cuotas"]:
            ctk.CTkLabel(sf, text=f"{campo} *").pack(pady=(5,0))
            e = ctk.CTkEntry(sf, width=300); e.pack(pady=2); entradas[campo] = e
            
        ctk.CTkLabel(sf, text="Frecuencia *").pack()
        frec_v = ctk.StringVar(value="Semanal"); ctk.CTkComboBox(sf, values=["Semanal", "Quincenal", "Mensual"], variable=frec_v, width=300).pack()
        lbl_v = ctk.CTkLabel(sf, text="CUOTA: $ 0,00", font=("", 20, "bold"), text_color="#2e7d32"); lbl_v.pack(pady=20)
        ...
        def guardar():
            ...
            p = float(limpiar_monto(entradas["Precio"].get())); c = int(entradas["Cuotas"].get()); v_c = p/c
            ...
            self.df_datos.to_excel(ARCHIVO_EXCEL, index=False)
            self.actualizar_lista(); vent.destroy(); messagebox.showinfo("Éxito", "Venta guardada.")
